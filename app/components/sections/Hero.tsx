...
const socialLinkClass = `
  relative group z-20 w-full sm:w-auto
  rounded-full border border-solid border-black/[.08] dark:border-white/[.145]
  transition-colors flex items-center justify-center
  hover:bg-[#f2f2f2] dark:hover:bg-[#1a1a1a]
  h-10 sm:h-12 px-2 sm:px-5 text-xs sm:text-base
`

export default function Hero() {
  return (
    <section id="home" className="grid w-full max-w-full grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 overflow-x-clip sm:p-20 font-[family-name:var(--font-body)]">
      {/* <FloatingElements /> */}
      <main className="flex w-full max-w-4xl flex-col gap-8 row-start-2 items-center">
        <AnimatedText className="flex w-full flex-col items-center gap-4">
          <Image
        ...
        <AnimatedText
          className="grid w-full max-w-sm grid-cols-2 gap-3 sm:flex sm:w-auto sm:max-w-none sm:gap-4"
        >
          <a
